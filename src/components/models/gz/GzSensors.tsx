...
import { TextField, Box, Button, MenuItem, Collapse, Typography, IconButton } from '@mui/material';
import DeleteIcon from '@mui/icons-material/Delete';
import ExpandMoreIcon from '@mui/icons-material/ExpandMore';
import ExpandLessIcon from '@mui/icons-material/ExpandLess';
import { Pose, Vector3 } from '../../../types';
...
export const SensorPanel: React.FC<SensorPanelInterface> = ({ sensors, setSensors }) => {
  ...
  const updateSensor = (id: string, field: string, value: unknown) => {
                ...
                <Box display="flex" gap={1}>
                  {['x', 'y', 'z'].map((axis) => (
                    <TextField
                      ...
                      type="number"
                      value={(sensor.pose.position as unknown as Record<string, number>)[axis]}
                      onChange={(e) => {
                ...
                <Box display="flex" gap={1}>
                  {['x', 'y', 'z', 'w'].map((axis) => (
                    <TextField
                      ...
                      label={axis.toUpperCase()}
                      type="number"
                      value={(sensor.pose.orientation as unknown as Record<string, number>)[axis]}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value);
                        const newPosition = {
                          ...sensor.pose,
                  ...
                  {['linear_noise', 'angular_noise'].map((noiseType) => (
                    <Box key={noiseType} mt={2}>
                      ...
                      {['mean', 'stddev', 'bias_mean', 'bias_stddev'].map((param) => (
                        <Box key={param} mt={1}>
                          <Typography variant="body2">{param.replace(/_/g, ' ')}</Typography>
                          <Box display="flex" gap={1}>
                            {['x', 'y', 'z'].map((axis) => (
                              <TextField
                                key={axis}
                                ...
                                value={
                                  (
                                    (sensor as unknown as Record<string, unknown>)[noiseType] as
                ...
                <Box>
                  ...
                  <Box display="flex" gap={2}>
                    <TextField
                      label="Mean"
                      type="number"
                      value={(sensor as AISSensor).noise?.mean ?? 0.01}
                      onChange={(e) => {
                        updateSensor(sensor.id, 'noise', {
                          ...(sensor as AISSensor).noise,
                    ...
                    <TextField
                      label="Stddev"
                      type="number"
                      value={(sensor as AISSensor).noise?.stddev ?? 0.0}
                      onChange={(e) => {
                        updateSensor(sensor.id, 'noise', {
                          ...(sensor as AISSensor).noise,
