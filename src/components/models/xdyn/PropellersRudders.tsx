...
export const PropellerWithRudderPanel: React.FC<PropellerWithRuddersPanelProps> = ({
  ...
  const handleChange = (
    ...
    const item = updated[index] as Record<keyof PropellerWithRudder, unknown>;
  ...
  const handleAdd = () => {
    setPropellerWithRudders([
      ...propellerWithRudders,
      {
        ...defaultPropellerWithRudder,
        name: `propeller_with_rudder_${propellerWithRudders.length}`,
      ...
      {propellerWithRudders.map((item, index) => (
        ...
        >
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            ...
            >
              <Typography sx={{ flexGrow: 1 }}>{item.name || `Fill in actuator name`}</Typography>
              <IconButton
                edge="end"
                color="error"
                onClick={(e) => {
              ...
              <Box display="flex" gap={1} flexWrap="wrap">
                {(['x', 'y', 'z', 'w'] as const).map((axis) => (
                  <TextField
                    ...
                    onChange={(e) => {
                      const updated = [...propellerWithRudders];
                      updated[index].propellerPose.orientation[axis] =
