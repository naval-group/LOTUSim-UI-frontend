...
export const PropellerPanel: React.FC<PropellerPanelProps> = ({ propellers, setPropellers }) => {
  const handleChange = (index: number, field: keyof Propeller, value: string | number) => {
    ...
    const item = updated[index] as Record<keyof Propeller, unknown>;
    ...
    if (field === 'name') {
      ...
      const isDuplicate = updated.some((prop, i) => i !== index && prop.name === name);
  ...
  const handleAdd = () => {
    setPropellers([
      ...
      {
        ...defaultPropeller,
        name: `propeller_${propellers.length}`,
        pose: { position: { x: 0, y: 0, z: 0 }, orientation: { x: 0, y: 0, z: 0, w: 1 } },
      },
    ]);
  };
